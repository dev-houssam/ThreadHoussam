...
void __attribute__((destructor)) cleanup() {
    ...
    */
    //thread_create(&afficherPerson, (void *) &personne3);
        ...
        //Person personne1 = {.nom="Stellare"};personne1;
    Person personne2 = {.nom="Sam"};
    Person personne3 = {.nom="Rachid"};
}

Person _c(const char * name){
    static Person p;
    sprintf(p.nom, "%s", name);
    return p;
}

#define go(X, A) thread_create(& X, (void *)  &(A))

#define new_object(name) (_c(name))

int main()
{
    printf("Hello World in this sequential thread truc !!!\n");
    Person personne1 = new_object("Chams");
    Person personne2 = new_object("sam");
    Person personne3 = new_object("Rachid");
    
    int i = 2;

    go(afficherPerson, personne1);
    go(afficherPerson, personne2);
    go(afficherPerson, personne3);
